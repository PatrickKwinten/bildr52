...
public class CommentController implements Serializable{
	...
	public ArrayList<JsonJavaObject> loadComments(String parentID) throws NotesException {
		AppController.writeToConsole(this.getClass().getSimpleName().toString() + ". loadComments(String)" + parentID);	
		ArrayList<JsonJavaObject> comments = new ArrayList<JsonJavaObject>();
		//get Info about Environment
		Database dataDB = dao.getDatabase();
		String ViewName = "$v-commentsJSON-multi";		
		if (!(dataDB==null)) {
			if ((dataDB.isOpen())) {
				AppController.writeToConsole(this.getClass().getSimpleName().toString() + ". loadComments(). DB found");	
			}
			else{
				XspOpenLogUtil.logError(null, null, this.getClass().getSimpleName().toString() + ". loadComments(). DB not found.", Level.SEVERE, null);
			}
		}				
		try {
			comments = JSFUtil.loadJSONObjects(dataDB.getServer(), dataDB.getFilePath(), ViewName, parentID, 2);
		} catch (NotesException e) {
			XspOpenLogUtil.logError(null, e, this.getClass().getSimpleName().toString() + ". loadComments().", Level.SEVERE, null);
		}
		dataDB.recycle();
		AppController.writeToConsole(this.getClass().getSimpleName().toString() + ". loadComments(). Array size = " + comments.size());	
		return comments;
	}
	
	public ArrayList<JsonJavaObject> loadComments() throws NotesException {
		AppController.writeToConsole(this.getClass().getSimpleName().toString() + ". loadComments()");
		ArrayList<JsonJavaObject> comments = new ArrayList<JsonJavaObject>();
		Database dataDB = dao.getDatabase();
		String ViewName = "$v-commentsJSON-flat";
		...
		try {
			comments = JSFUtil.loadJSONObjects(dataDB.getServer(), dataDB.getFilePath(), ViewName, "", 1);
		} catch (NotesException e) {
			XspOpenLogUtil.logError(null, e, this.getClass().getSimpleName().toString() + ". loadComments().", Level.SEVERE, null);
		}
		dataDB.recycle();
		...
		if (getDebugMode() == true){
			DebugToolbarBean.get().info( comments );
			DebugToolbarBean.get().addDivider();
			DebugToolbarBean.get().warn( comments );
		}	
		return comments;
	}
	
	public ArrayList<JsonJavaObject> searchJSONObjects(String searchKey, Integer resultLimit) throws NotesException {
		AppController.writeToConsole(this.getClass().getSimpleName().toString() + ". searchJSONObjects(String searchKey, Integer resultLimit)");
