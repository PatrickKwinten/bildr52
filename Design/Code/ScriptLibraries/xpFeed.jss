...
FEED_TITLE = database.getTitle()
FEED_SUBTITLE = "...";
FEED_LINK = protocolOnly.toLowerCase() + "://" + server + port + con.getRequestContextPath() + "/feed.xsp"
FEED_LINK_SELF = "feed.xsp";
FEED_STD_ENTRY_COUNT = 40; // used, if no "count" URL parameter is set
FEED_FORMAT_ATOM = "atom";
FEED_FORMAT_RSS = "rss";


FEED_SOURCE_VIEW = "$v-feed"
...
function NotesDataService()
{
	...
	this.prototype.getFeedInfo = function()
	{
		...
		var feedView = db.getView(FEED_SOURCE_VIEW);
	...
	this.prototype.getFeedEntries = function(entryCount)
	{
		...
		//var feedView = database.getView("Feed");
		var feedView = db.getView(FEED_SOURCE_VIEW);
		...
		while (i < entryCount && entry != null) {
			...
			feedEntry["Abstract"] = colValues.elementAt(7);
